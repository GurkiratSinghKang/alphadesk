...
def _define_models() -> dict[str, Any]:
    ...
    def _normalize_symbol(value):
        """Round-29 / persona-E P0: uppercase + strip whitespace at the
        ORM write boundary. Pre-fix every model with a ``symbol`` column
        was case-sensitive at the DB level (Postgres ``String`` is
        case-sensitive by default; ``Position.symbol`` is even
        ``unique=True``), so two clients writing "AAPL" and "aapl"
        could create duplicate rows for the same economic position.
        Full ``CITEXT`` migration is gated on the multi-tenant schema
        rework — this validator is the interim fix that prevents drift
        on every new write without touching the existing schema.
        """
    ...
    class OHLCVBar(Base):
        ...
        @validates("symbol")
        def _validate_symbol(self, _key, value):
    ...
    class Trade(Base):
        ...
        __table_args__ = (
            ...
            # WHERE symbol = (param) ORDER BY entry_time DESC used by
