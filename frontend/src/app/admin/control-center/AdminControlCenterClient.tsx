...
function DeployPanel() {
  ...
  useEffect(() => {
    ...
    getLastDeploy()
      .then((data) => {
        if (!cancelled) setLast(data);
      })
      .catch((err: Error) => {
        if (!cancelled) setDeployError(err.message || "Admin deploy status unavailable");
      })
      .finally(() => {
  ...
  const onTrigger = async () => {
    if (loadingLast || deployError) return;
    ...
    >
      <div className="flex flex-wrap items-center gap-3">
        <button
          type="button"
          onClick={onTrigger}
          disabled={triggering || loadingLast || Boolean(deployError)}
          className="min-h-touch rounded border border-[color:var(--brand)] bg-[color:var(--brand-tint)] px-4 t-mono text-label u-brand disabled:opacity-50"
        >
          {triggering ? "Triggering…" : loadingLast ? "Checking admin…" : deployError ? "Deploy locked" : "▸ Push to prod"}
        </button>
        {deployError ? (
          <p className="t-mono text-label u-loss">
            Admin deploy controls unavailable: {deployError}
          </p>
        ) : null}
        {last?.triggered_at ? (
