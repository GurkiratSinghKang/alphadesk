...
export default function SavedViewsBar<T>({
  ...
  return (
    ...
    >
      <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-[color:var(--fg-muted)] mr-1">
        Views
      </span>
      {views.length === 0 ? (
        <span className="font-mono text-[11px] text-[color:var(--fg-muted)] italic">
          — no saved views
        </span>
      ) : (
        views.map((v) => (
      ...
      <button
        type="button"
        onClick={handleSave}
        className="ml-auto inline-flex items-center gap-1 rounded border border-dashed border-[color:var(--border)] px-2 py-0.5 t-mono text-[11.5px] text-[color:var(--fg-muted)] hover:border-[color:var(--brand)] hover:text-[color:var(--brand)] transition-colors"
