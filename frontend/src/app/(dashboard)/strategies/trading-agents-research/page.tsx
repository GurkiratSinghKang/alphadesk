...
function levelToneClass(tone: LevelTone) {
  ...
  if (tone === "current") return "border-primary/35 bg-primary/10";
...
export default function TradingAgentsResearchPage() {
  ...
  return (
    ...
    >
       ...
       * established across /strategies/earnings-options-play +
       * /strategies/[id]/playbook + /reports + /alerts + /watchlists.
       * The READ-ONLY pill is folded into the hero (right-aligned)
       * instead of floating above it, matching the EOP hero pattern
       * where the live status chip sits inside the same identity band. */}
      <header
      ...
      >
        ...
        {/* h1 (not h2) because DashboardPageLayout above is mounted with
            ...
            heading. Other v2 dashboard pages keep h2 here because they
            nest under a chrome <h1>; here we own the h1 — same pattern
            as /strategies/earnings-options-play. */}
        <h1
          ...
          style={{
            ...
            // /strategies/earnings-options-play + /strategies/[id]/playbook
            // + /backtest. Previously 30px which made this hero out-
            // shout the identity-band eyebrow above.
            fontSize: 28,
            fontWeight: 400,
    ...
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        <button
          ...
          className={cn(
            "inline-flex h-10 items-center gap-2 rounded-md border border-primary bg-primary px-4",
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Symbol">
          <input
            ...
            className="h-10 rounded-md border border-border bg-bg-elev-1 px-3 font-mono text-body text-fg outline-none transition focus:border-primary"
            maxLength={12}
            required
          />
        </Field>
        ...
        <Field label="Provider">
          <select
            ...
            className="h-10 rounded-md border border-border bg-bg-elev-1 px-3 font-sans text-body-sm text-fg outline-none transition focus:border-primary"
          >
            {PROVIDERS.map((item) => (
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Field label="Deep model">
            <input
              ...
              className="h-10 rounded-md border border-border bg-bg-card px-3 font-mono text-body-sm text-fg outline-none transition focus:border-primary"
            />
          </Field>
      ...
      <Field label="Research note" className="mt-3">
        <textarea
          ...
          className="resize-none rounded-md border border-border bg-bg-elev-1 px-3 py-2 font-sans text-body-sm leading-relaxed text-fg outline-none transition focus:border-primary"
        />
      </Field>
    </form>
  );
        ...
        {highlights.map((line, index) => (
          ...
          >
            <div className="flex gap-3">
              <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-primary/35 bg-primary/10 font-mono text-eyebrow text-primary">
                {index + 1}
              </span>
    ...
    <section className="rounded-lg border border-border-hair bg-bg-elev-1 p-4">
      ...
      <nav className="space-y-1" aria-label="TradingAgents memo sections">
        {displaySections.map((section, index) => (
          ...
          >
            <span className="break-words text-body-sm leading-relaxed text-fg group-hover:text-primary">{section.title}</span>
  ...
  return (
    <div className="space-y-3">
      {rows.map((line, index) => {
        ...
        if (/^#{3,6}\s+/.test(line)) {
          return (
            <p key={`${index}-${line}`} className="pt-3 t-label text-primary">
              {clean}
            </p>
          );
        }
        ...
        if (numbered) {
          return (
            <div key={`${index}-${line}`} className="flex gap-3">
              <span className="mt-0.5 w-6 shrink-0 font-mono text-label text-primary">{numbered[1]}.</span>
              <p className="min-w-0 break-words text-body leading-7 text-fg">
...
function ArtifactPanel({ files }: { files: string[] }) {
  return (
    <section className="rounded-lg border border-border-hair bg-bg-elev-1 p-4">
      <div className="mb-3 flex items-center gap-2">
        <Files className="h-4 w-4 text-primary" weight="bold" />
...
function ThinMemoPanel({ run, signal }: { run: TradingAgentsRun; signal: string }) {
  return (
    <section className="rounded-lg border border-primary/35 bg-primary/10 p-5">
      <p className="t-label text-primary">Processed signal only</p>
    ...
    <div className="flex min-h-[520px] items-center justify-center rounded-lg border border-dashed border-border-hair bg-bg-card p-8 text-center">
      <div className="max-w-md">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border border-border bg-bg-elev-1">
          <Scales className="h-5 w-5 text-primary" weight="bold" />
        </div>
          ...
          {warnings.length > 0 && (
            <ul className="mt-3 flex flex-col gap-2">
              {warnings.map((warning) => (
                <li key={warning} className="rounded-md border border-primary/40 bg-primary/10 px-3 py-2 text-label leading-relaxed text-fg">
                  {warning}
                </li>
              ))}
            </ul>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  selected ? "border-primary/70 bg-primary/10" : "border-border-hair bg-bg-elev-1 hover:border-border",
                )}
              >
  ...
  return (
    <div className="rounded-md border border-border-hair bg-bg-elev-1 px-3 py-2">
      <div className="flex items-center gap-1.5">
        <Icon className="h-3.5 w-3.5 text-primary" weight="bold" />
