...
/**
 * AlphaDesk InputGroup
 * ────────────────────
 * The 36px editorial input row with inline-leading and inline-trailing
 * adornments (e.g. leading `$`, trailing `shares`, or an icon). Mirrors
 * `components-inputs.html` — the border sits on the group wrapper, and the
 * inner <Input> drops its own border so adornments appear inside one frame.
 *
 * Gold focus ring is triggered when the inner control is focused.
 */
...
const inputGroupAddonVariants = cva(
  [
    ...
    "font-mono text-label tracking-[0.08em] uppercase text-fg-muted",
    "select-none",
  ...
  ].join(" "),
  {
    variants: {
      align: {
        "inline-start":
          "order-first px-3 border-r border-border has-[>button]:ml-[-0.3rem] has-[>kbd]:ml-[-0.15rem] has-[>svg]:border-r-0 has-[>svg]:pr-1",
        "inline-end":
          "order-last px-3 border-l border-border has-[>button]:mr-[-0.3rem] has-[>kbd]:mr-[-0.15rem] has-[>svg]:border-l-0 has-[>svg]:pl-1",
        "block-start":
