...
class PairsTradingStrategy(Strategy):
    ...
    def run(
        ...
        exits = _compute_exits(closes, positions, active, params, asof)
        ...
        entries = _compute_entries(
            closes, positions, active, params, asof, exclude_pair_ids=closed_pair_ids,
        )
        diagnostics["entries_emitted"] = len(entries)

        held_symbols = {p.y for p in positions.values()} | {p.x for p in positions.values()}
...
def _close_matrix(
    ...
    frame["ts"] = pd.to_datetime(frame["ts"], utc=False, errors="coerce")
...
def _run_watchdog(
    ...
    """Re-test cointegration on active pairs due for watchdog; return failing ids."""
    due = [
...
def _compute_exits(
    ...
    watchdog_failures = _run_watchdog(closes, active, params, asof)
...
def _compute_entries(
    ...
    if len(positions) >= params.max_pairs:
        return []
    held_syms = {pos.y for pos in positions.values()} | {pos.x for pos in positions.values()}
    exclude = exclude_pair_ids or set()

    candidates: list[tuple[float, ActivePair, float, float, float, float]] = []
    for pair in active:
        if pair.pair_id in positions or pair.pair_id in exclude:
            continue
    ...
    entries: list[Signal] = []
    remaining = params.max_pairs - len(positions)
