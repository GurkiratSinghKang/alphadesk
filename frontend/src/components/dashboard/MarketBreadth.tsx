  ...
  if (total === 0) {
    return (
      <div className="rounded-xl border border-border bg-[var(--panel)]">
        ...
        <div className="p-4">
          <p className="text-label text-muted-foreground">Loading sector data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-border bg-[var(--panel)]">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        ...
        <span
          className={cn(
            "text-label font-bold uppercase px-2 py-0.5 rounded border",
      ...
      <div className="px-4 pt-3 mb-3">
        ...
        <div className="flex justify-between mt-1">
          <span className="text-label text-[var(--loss)]">Bearish</span>
          <span className="text-label text-muted-foreground">Neutral</span>
          <span className="text-label text-[var(--profit)]">Bullish</span>
        </div>
      </div>
      ...
      {sectors.length > 0 && (
        <div className="mx-4 mt-3 pt-3 pb-4 border-t border-border">
              ...
              .map((s) => {
                ...
                return (
                  <span
                    key={s.sector}
                    className={cn(
                      "text-label px-1.5 py-0.5 rounded border",
                      positive
          ...
          <p className="text-label text-muted-foreground/50 mt-2">Breadth measured across GICS sectors, not individual stocks.</p>
        </div>
      )}
    </div>
  );
